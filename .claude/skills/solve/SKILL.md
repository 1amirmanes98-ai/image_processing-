---
name: solve
description: Socratic step-by-step walkthrough of a specific past exam question (FODL or Reinforcement Learning). Use for "/solve a_2023 2", "/solve b_2023 q1", "walk me through question 3 of the 2022 exam".
---
...
empty, list available exams from `<AGENT>/index/EXAM_MAP.md` with one-line
question summaries and let the user pick.

## Flow

1. Load the question from `<AGENT>/index/exams/` — the file is `fodl_exam_<id>.md` for
   FODL and `<id>.md` (e.g. `a_2023.md`, `example.md`) for RL. If any wording
   seems off, verify against the PDF (`<AGENT>/materials/exams/`, Read pages).
